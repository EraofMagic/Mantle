...
public class DumpAllTagsCommand {
  ...
  private static int runAll(CommandContext<CommandSourceStack> context) {
    ...
    context.getSource().sendSuccess(() -> Component.translatable("command.eomantle.dump_all_tags.success", getOutputComponent(output)), true);
    return tagsDumped;
  }
  ...
  private static int runType(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
    ...
    // print result
    context.getSource().sendSuccess(() -> Component.translatable("command.eomantle.dump_all_tags.type_success", registry.key().location(), getOutputComponent(output)), true);
    return result;
  }
