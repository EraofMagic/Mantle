...
public class ViewTagCommand {
  ...
  protected static final Dynamic2CommandExceptionType TAG_NOT_FOUND = new Dynamic2CommandExceptionType((type, name) -> Component.translatable("command.eomantle.tag.not_found", type, name));
  ...
  private static <T> int runGeneric(CommandContext<CommandSourceStack> context, Registry<T> registry) throws CommandSyntaxException {
    ...
    if (holder != null) {
      ...
      MutableComponent output = Component.translatable("command.eomantle.view_tag.success", registry.key().location(), name);
