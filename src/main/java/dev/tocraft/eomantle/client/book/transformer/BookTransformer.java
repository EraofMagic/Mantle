package dev.tocraft.eomantle.client.book.transformer;
...
import dev.tocraft.eomantle.client.book.data.content.ContentPadding.PaddingBookTransformer;
import dev.tocraft.eomantle.client.book.data.content.ContentTableOfContents;
      ...
      for (SectionData section : book.sections) {
        ...
        for (int i = 0; i < pages.length; i++) {
          ...
          for (int j = 0; j < text.length; j++) {
            ...
            text[j].action = "eomantle:go-to-page-rtn " + section.name + "." + sectionPages.get(i * ENTRIES_PER_PAGE + j).name;
          }
